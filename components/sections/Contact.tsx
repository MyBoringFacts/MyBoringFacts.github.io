"use client";

import { useTheme } from "@/components/theme-provider";
import { CardSpotlight } from "@/components/ui/card-spotlight";
import { motion } from "motion/react";
import { useInView } from "motion/react";
import { useRef } from "react";

const contacts = [
  {
    label: "Email",
    value: "thadoe.work@gmail.com",
    href: "mailto:thadoe.work@gmail.com",
    icon: (
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        <rect x="2" y="4" width="20" height="16" rx="2" />
        <path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7" />
      </svg>
    ),
  },
  {
    label: "LinkedIn",
    value: "linkedin.com/in/thadoe",
    href: "https://www.linkedin.com/in/thadoe/",
    icon: (
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        <path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z" />
        <rect width="4" height="12" x="2" y="9" />
        <circle cx="4" cy="4" r="2" />
      </svg>
    ),
  },
  {
    label: "GitHub",
    value: "github.com/MyBoringFacts",
    href: "https://github.com/MyBoringFacts",
    icon: (
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        <path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4" />
        <path d="M9 18c-4.51 2-5-2-7-2" />
      </svg>
    ),
  },
];

export function ContactSection() {
  const { theme } = useTheme();
  const isColorful = theme === "black-hole";
  const ref = useRef(null);
  const isInView = useInView(ref, { once: true, margin: "-100px" });

  return (
    <section id="contact" className="relative py-32 px-4 md:px-8" ref={ref}>
      <div className="max-w-4xl mx-auto">
        <motion.div
          initial={{ opacity: 0, y: 40 }}
          animate={isInView ? { opacity: 1, y: 0 } : {}}
          transition={{ duration: 0.8 }}
          className="text-center mb-12"
        >
          <h2
            className="text-4xl md:text-6xl font-bold mb-4"
            style={{ color: isColorful ? "#fafafa" : "#171717" }}
          >
            Get in Touch
          </h2>
          <div
            className="h-1 w-20 mb-8 mx-auto"
            style={{ backgroundColor: isColorful ? "#06b6d4" : "#404040" }}
          />
          <p
            className="text-lg leading-relaxed max-w-2xl mx-auto"
            style={{ color: isColorful ? "#a3a3a3" : "#525252" }}
          >
            {`I'm`} always interested in hearing about new opportunities, 
            collaborations, or just having a chat about AI and technology. 
            Feel free to reach out!
          </p>
        </motion.div>

        <div className="grid sm:grid-cols-2 gap-6">
          {contacts.map((contact, idx) => (
            <motion.div
              key={contact.label}
              initial={{ opacity: 0, y: 20 }}
              animate={isInView ? { opacity: 1, y: 0 } : {}}
              transition={{ duration: 0.5, delay: 0.4 + idx * 0.1 }}
            >
              <CardSpotlight
                color={isColorful ? "#262626" : "#d4d4d4"}
                className="h-full w-full rounded-2xl p-0 transition-all duration-300 hover:scale-[1.02]"
              >
                <a
                  href={contact.href}
                  target={contact.href.startsWith("http") ? "_blank" : undefined}
                  rel={contact.href.startsWith("http") ? "noopener noreferrer" : undefined}
                  className="relative z-10 flex items-center gap-4 p-6"
                >
                  <div
                    className="rounded-xl p-3"
                    style={{
                      backgroundColor: isColorful ? "rgba(6, 182, 212, 0.2)" : "#f5f5f5",
                      color: isColorful ? "#06b6d4" : "#404040",
                    }}
                  >
                    {contact.icon}
                  </div>
                  <div>
                    <p
                      className="text-xs font-mono uppercase tracking-wider"
                      style={{ color: isColorful ? "#737373" : "#a3a3a3" }}
                    >
                      {contact.label}
                    </p>
                    <p
                      className="font-medium"
                      style={{ color: isColorful ? "#fafafa" : "#171717" }}
                    >
                      {contact.value}
                    </p>
                  </div>
                </a>
              </CardSpotlight>
            </motion.div>
          ))}
        </div>

        <motion.div
          initial={{ opacity: 0, y: 40 }}
          animate={isInView ? { opacity: 1, y: 0 } : {}}
          transition={{ duration: 0.8, delay: 0.6 }}
          className="mt-24 pt-12 border-t text-center"
          style={{ borderColor: isColorful ? "#262626" : "#e5e5e5" }}
        >
          <p
            className="text-sm"
            style={{ color: isColorful ? "#737373" : "#a3a3a3" }}
          >
            © 2026 Thadoe Hein. Built with Next.js & Motion.
          </p>
        </motion.div>
      </div>
    </section>
  );
}
